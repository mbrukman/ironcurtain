...
export interface SplashScreen {
  ...
  stop(): void;
  draw(): void;
  resize(cols: number, viewportRows: number, viewportY: number): void;
...
export function createSplashScreen(
  ...
  function init(): void {
    _layout = computeWordLayout(_cols, _viewportRows);
    if (_layout) {
  ...
  /** Returns a buffer pre-filled with escape sequences to clear every viewport row. */
  function clearViewport(): string[] {
    const buf: string[] = [];
    for (let r = 0; r < _viewportRows; r++) {
    ...
    if (!_done) {
      for (const rc of _rain) {
        ...
        for (let d = 0; d <= TRAIL_LEN; d++) {
          const y = rc.headY - d;
          if (y < 0 || y >= _viewportRows) continue;
  ...
  function renderFallback(): void {
    ...
    const y = Math.floor(_viewportRows / 2);
  ...
  function renderInfo(): void {
    ...
    // Header
    if (infoY < _viewportY + _viewportRows) {
      buf.push(
    ...
    for (const line of commands) {
      if (y >= _viewportY + _viewportRows) break;
      if (!line) {
        y++;
        continue;
      }
